...
function MetricCard({ project }: { project: Project }) {
  const accent = accentColors[project.slug] ?? "#c9ff6a";
  return (
    <div className="border border-white/[0.07] bg-white/[0.018] p-5 sm:p-6">
      <p className="font-mono text-[9px] uppercase tracking-widest mb-5" style={{ color: "rgba(247,247,242,0.25)" }}>
        Proof points
      </p>
        ...
        {project.caseStudy.metrics.map((metric, i) => (
          <div key={metric.label}>
            <p
              ...
              style={{ color: i % 2 === 0 ? accent : "rgba(247,247,242,0.85)" }}
            >
              {metric.value}{metric.suffix ?? ""}
            </p>
            <p className="font-mono text-[9px] uppercase tracking-wide leading-relaxed" style={{ color: "rgba(247,247,242,0.30)" }}>
              {metric.label}
            </p>
          </div>
        ))}
...
function WorkShowcase({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <ScrollReveal delay={index * 0.07}>
      <article className="border-t border-white/[0.07] py-14 sm:py-18 lg:py-24">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:gap-16 lg:items-start">
          ...
          <div className="lg:sticky lg:top-24 flex flex-col gap-7">
            ...
            <div className="flex items-center gap-4">
              <span className="font-mono text-[10px] tabular-nums" style={{ color: "rgba(247,247,242,0.20)" }}>
            ...
            {/* Title */}
            <div>
              <h2 className="text-[2.6rem] font-black leading-[0.9] text-white sm:text-5xl lg:text-[3.5rem] text-balance">
                {project.title}
              </h2>
              <p className="mt-4 text-[0.9375rem] leading-relaxed max-w-sm" style={{ color: "rgba(247,247,242,0.48)" }}>
                {project.pitch}
              </p>
            </div>
            ...
            {/* Metadata table */}
            <dl className="grid grid-cols-2 gap-x-6 gap-y-4 border-t border-white/[0.06] pt-5">
              {[
                ["Role", project.role],
                ...
                <div key={label}>
                  <dt className="font-mono text-[9px] uppercase tracking-widest mb-1" style={{ color: "rgba(247,247,242,0.22)" }}>
                    {label}
                  </dt>
                  <dd className="font-mono text-[11px] leading-relaxed" style={{ color: "rgba(247,247,242,0.55)" }}>
                    {value}
                  </dd>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                ...
                className="tap-target focus-ring inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-black transition-all duration-200 hover:bg-[#c9ff6a]"
              >
                Case study
              ...
              {project.liveUrl && (
                <a
                  ...
                  className="tap-target focus-ring inline-flex items-center gap-2 rounded-full border border-white/[0.12] px-5 py-2.5 text-sm font-semibold transition-colors duration-200 hover:border-white/28 hover:text-white"
            ...
            <div className="relative aspect-[16/9] overflow-hidden border border-white/[0.08]">
              {heroImage ? (
                ...
                >
                  ...
                  <p className="relative text-3xl font-black text-white/20">{project.title}</p>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {screens.map((screen) => (
                  <div key={screen.title} className="flex flex-col gap-2">
                    <div className="relative aspect-[16/10] overflow-hidden border border-white/[0.07]">
                      {screen.image ? (
                        <Image
                          src={screen.image}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-white/[0.025]">
                          <p className="font-mono text-[9px] uppercase text-white/25 text-center px-4">
                            {screen.title}
                          </p>
                        </div>
                      )}
                    </div>
                    <p className="font-mono text-[9px] leading-relaxed" style={{ color: "rgba(247,247,242,0.25)" }}>
                      {screen.caption}
                    </p>
                  </div>
                ))}
