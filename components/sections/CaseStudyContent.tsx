...
function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <p className="font-mono text-[10px] text-white/25 tracking-widest uppercase mb-4">
      {children}
    </p>
  );
}
...
function SectionHeading({ children }: { children: ReactNode }) {
  return (
    <h2 className="font-display text-3xl md:text-4xl text-white mb-8 sm:mb-10 leading-tight text-balance">
      {children}
    </h2>
  );
}
...
function MediaFrame({
  ...
  return (
    <div
      className={`relative w-full overflow-hidden rounded-sm border border-white/[0.08] bg-[#0A0A0F] ${aspectClasses[aspect]}`}
    >
      {image ? (
        <Image
          src={image}
...
function PersonaCard({ persona, index }: { persona: Persona; index: number }) {
  const accentColor = index === 0 ? "#c9ff6a" : "#f3a841";
  return (
    <ScrollReveal delay={index * 0.1}>
      <div className="border border-white/[0.08] bg-white/[0.02] p-6 sm:p-8">
        {/* Header */}
        <div className="flex items-start justify-between gap-4 border-b border-white/[0.06] pb-5 mb-6">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-widest mb-1" style={{ color: `${accentColor}80` }}>
              {persona.type}
            </p>
            <h3 className="text-xl font-black text-white leading-none">{persona.name}</h3>
            <p className="font-mono text-xs mt-1" style={{ color: "rgba(247,247,242,0.42)" }}>
              {persona.age} · {persona.location}
            </p>
            <p className="font-mono text-[10px] mt-0.5" style={{ color: "rgba(247,247,242,0.32)" }}>
              {persona.role}
            </p>
          </div>
          <div
        ...
        {/* Goals */}
        <div className="mb-5">
          <p className="font-mono text-[9px] uppercase tracking-widest mb-3" style={{ color: "rgba(247,247,242,0.25)" }}>
            Goals
          </p>
          <ul className="space-y-2">
            {persona.goals.map((goal, i) => (
              <li key={i} className="flex items-start gap-2.5">
                ...
                <span className="text-xs leading-relaxed" style={{ color: "rgba(247,247,242,0.55)" }}>{goal}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="mb-5">
          ...
          <ul className="space-y-2">
            {persona.frustrations.map((f, i) => (
              <li key={i} className="flex items-start gap-2.5">
                <span className="mt-1.5 w-1 h-1 rounded-sm flex-shrink-0 bg-white/20" />
                <span className="text-xs leading-relaxed" style={{ color: "rgba(247,247,242,0.45)" }}>{f}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        >
          <p className="text-sm italic leading-relaxed" style={{ color: "rgba(247,247,242,0.60)" }}>
            {persona.quote}
          </p>
        </blockquote>
      </div>
...
function ColorSwatch({ token }: { token: ColorToken }) {
  return (
    <div className="flex items-center gap-4 border border-white/[0.06] p-4">
      <div
        className="w-12 h-12 flex-shrink-0 rounded-sm border border-white/[0.08]"
        style={{ background: token.hex }}
        aria-label={`Colour swatch for ${token.name}`}
      />
      <div className="min-w-0">
        <p className="text-sm font-semibold text-white leading-none mb-0.5">{token.name}</p>
        <p className="font-mono text-[10px] text-white/35 mb-1.5">{token.hex}</p>
        <p className="text-xs leading-relaxed" style={{ color: "rgba(247,247,242,0.45)" }}>{token.usage}</p>
      </div>
    </div>
  );
}
...
function TypographyRow({ item, index }: { item: TypographyItem; index: number }) {
  return (
    <div className="border-t border-white/[0.06] py-6 grid lg:grid-cols-[1fr_auto_2fr] gap-3 lg:gap-10 items-start">
      <div>
        <p className="text-lg font-bold text-white leading-none mb-1">{item.family}</p>
        <p className="font-mono text-[10px] text-white/30">{item.weights}</p>
      </div>
      <p className="font-mono text-[10px] text-white/20 tracking-widest uppercase lg:mt-1">
        {String(index + 1).padStart(2, "0")}
      </p>
      <p className="text-sm leading-relaxed" style={{ color: "rgba(247,247,242,0.55)" }}>
        {item.usage}
      </p>
    </div>
  );
          ...
          <ScrollReveal>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10">
              {caseStudy.metrics.map((metric, i) => (
                <div key={i}>
                  <div className="font-mono text-3xl sm:text-4xl md:text-5xl text-white tabular-nums leading-none mb-2">
                    <AnimatedNumber
                  ...
                  <div className="font-mono text-xs text-white/30 leading-relaxed max-w-[16ch]">
                    {metric.label}
                  </div>
                </div>
              ))}
        ...
        <Container size="lg">
          ...
          <ScrollReveal delay={0.1}>
            <blockquote className="mb-12">
              <p className="font-display text-2xl md:text-3xl lg:text-4xl text-white/80 leading-snug italic max-w-3xl">
                {caseStudy.pullQuote}
              </p>
            </blockquote>
          </ScrollReveal>
          ...
          <div className="grid lg:grid-cols-[2fr_1fr] gap-10 lg:gap-12">
            <ScrollReveal delay={0.15}>
              <p className="text-white/55 leading-relaxed text-[0.9375rem]">
                {caseStudy.brief}
              </p>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.2}>
              <div>
                <p className="font-mono text-[10px] text-white/25 tracking-widest uppercase mb-5">
                  Goals
                </p>
                <ul className="space-y-4">
                  {caseStudy.goals.map((goal, i) => (
                    <li key={i} className="flex items-start gap-4">
                      <span className="font-mono text-xs text-white/20 flex-shrink-0 mt-0.5 tabular-nums">
                        {String(i + 1).padStart(2, "0")}
                      </span>
                      <span className="text-sm text-white/50 leading-relaxed">{goal}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <Section>
        <Container size="lg">
          <ScrollReveal>
            ...
            <p className="text-white/55 leading-relaxed text-[0.9375rem] max-w-2xl mb-12">
          ...
          <div className="space-y-5 mb-14 max-w-2xl">
            {caseStudy.research.paragraphs.map((para, i) => (
              <ScrollReveal key={i} delay={i * 0.08}>
                <p className="text-white/45 leading-relaxed text-sm">{para}</p>
              </ScrollReveal>
            ))}
          </div>
          ...
          <ScrollReveal delay={0.15}>
            <p className="font-mono text-[10px] text-white/25 tracking-widest uppercase mb-8">
              Key Insights
            </p>
            <div className="space-y-7">
              {caseStudy.research.insights.map((insight, i) => (
                <div key={i} className="flex items-start gap-4 sm:gap-6 border-t border-white/[0.06] pt-7">
                  <span className="font-mono text-2xl md:text-3xl text-white/10 tabular-nums leading-none flex-shrink-0 w-10">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <p className="text-white/55 leading-relaxed text-[0.9375rem] max-w-xl">
                    {insight}
                  </p>
                </div>
              ))}
            ...
            <Container size="lg">
              ...
              {caseStudy.colorPalette && caseStudy.colorPalette.length > 0 && (
                <ScrollReveal delay={0.08}>
                  <div className="mb-14">
                    <p className="font-mono text-[10px] text-white/25 tracking-widest uppercase mb-6">
                      Colour Palette
                    </p>
          ...
          <div className="space-y-0">
            {caseStudy.keyDecisions.map((decision, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <div className="border-t border-white/[0.06] py-8 grid lg:grid-cols-[auto_1fr_2fr] gap-4 sm:gap-6 lg:gap-10">
                  <span className="font-mono text-2xl text-white/10 tabular-nums leading-none w-10 flex-shrink-0">
                    {String(i + 1).padStart(2, "0")}
                  </span>
          ...
          {caseStudy.beforeAfter ? (
            <ScrollReveal delay={0.12}>
              <div className="mb-12">
                ...
                <p className="font-mono text-xs text-white/25 italic mt-4 max-w-2xl">
              ...
              <ScrollReveal key={i} delay={i * 0.1}>
                <div>
                  ...
                  <div className="mt-4 grid md:grid-cols-[auto_1fr] gap-3 md:gap-6 max-w-3xl">
                    <p className="font-mono text-xs text-white/25 italic whitespace-nowrap">
                      Fig. {i + 1}
                    </p>
                    <div>
                      ...
                      {screen.decision ? (
                        <p className="text-xs text-white/35 leading-relaxed mt-3 border-l border-white/[0.08] pl-4">
                          <span className="font-mono text-white/25 tracking-widest uppercase">
                            Decision
                          </span>
        ...
        <Container size="lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
            {caseStudy.impact.map((item, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <div>
                  <div className="font-mono text-5xl md:text-6xl text-white leading-none mb-3">
                    {item.value}
                  </div>
                  <div className="text-sm font-medium text-white/60 mb-1">
                    {item.label}
                  </div>
                  <div className="text-xs text-white/30 leading-relaxed max-w-[22ch]">
                    {item.description}
                  </div>
                </div>
              </ScrollReveal>
